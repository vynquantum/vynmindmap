...
export async function getRecents(): Promise<RecentEntry[]> {
  ...
  return [...paths, ...handles].sort((a, b) => b.when - a.when).slice(0, MAX_RECENTS);
}
...
export async function addRecentHandle(handle: FsFileHandle, name: string): Promise<void> {
  ...
  const same = await Promise.all(
    rows.map(async (r) => {
      const sameEntry = (r.handle as { isSameEntry?: (o: unknown) => Promise<boolean> })
        .isSameEntry;
      try {
  ...
  if (after.length > MAX_RECENTS) {
    ...
    await idbTx(db, (store) => {
      for (const r of excess) store.delete(r.key);
    });
  }
  db.close();
}
  ...
  const same = await Promise.all(
    rows.map(async (r) => {
      const sameEntry = (r.handle as { isSameEntry?: (o: unknown) => Promise<boolean> })
        .isSameEntry;
      try {
        return sameEntry && entry.handle
          ? await sameEntry.call(r.handle, entry.handle)
          : r.name === entry.name;
      } catch {
